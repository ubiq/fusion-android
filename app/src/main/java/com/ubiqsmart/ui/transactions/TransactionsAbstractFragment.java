...
public abstract class TransactionsAbstractFragment extends Fragment implements View.OnClickListener, View.OnCreateContextMenuListener {
  ...
  // used to count to two (since internal and normal transactions are each one request). Gets icnreased once one request is finished. If it is two, notifyDataChange is called (display transactions)
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    ...
    send = rootView.findViewById(R.id.newTransaction);
    requestTx = rootView.findViewById(R.id.requestTx);
    ...
    recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
      @Override public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        } else if (dy < 0) fabmenu.showMenu(true);
      }
    });
  ...
  public synchronized void addToWallets(List<TransactionDisplay> w) {
    ...
    Collections.sort(getWallets(), new Comparator<TransactionDisplay>() {
      @Override public int compare(TransactionDisplay o1, TransactionDisplay o2) {
