...
public class FragmentTransactionsAll extends FragmentTransactionsAbstract {
        ...
        try {
          ...
          EtherscanAPI.getInstance().getNormalTransactions(currentWallet.getPubKey(), new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
              ...
              final ArrayList<TransactionDisplay> w = new ArrayList<TransactionDisplay>(
                  ResponseParser.parseTransactions(restring, "Unnamed Address", currentWallet.getPubKey(), TransactionDisplay.NORMAL));
              if (isAdded()) {
                ac.runOnUiThread(new Runnable() {
                  @Override public void run() {
          ...
          EtherscanAPI.getInstance().getInternalTransactions(currentWallet.getPubKey(), new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
              ...
              final ArrayList<TransactionDisplay> w = new ArrayList<TransactionDisplay>(
                  ResponseParser.parseTransactions(restring, "Unnamed Address", currentWallet.getPubKey(), TransactionDisplay.CONTRACT));
              if (isAdded()) {
                ac.runOnUiThread(new Runnable() {
                  @Override public void run() {
                    onComplete(w, storedwallets);
          ...
          if (isAdded()) {
            if (ac != null) ((MainActivity) ac).snackError("Can't fetch account balances. No connection?");
  ...
  private void onComplete(ArrayList<TransactionDisplay> w, ArrayList<StorableWallet> storedwallets) {
    ...
    if (getRequestCount() >= storedwallets.size() * 2) {
      ...
      if (unconfirmed_addedTime + 10 * 60 * 1000
          < System.currentTimeMillis()) // After 10 minutes remove unconfirmed (should now have at least 1 confirmation anyway)
      {
        unconfirmed = null;
      }
      if (unconfirmed != null) {
