...
public class TransactionAdapter extends RecyclerView.Adapter<TransactionAdapter.MyViewHolder> {
  ...
  public TransactionAdapter(List<TransactionDisplay> boxlist, Context context, View.OnClickListener clickListener,
  ...
  @Override public void onBindViewHolder(final MyViewHolder holder, final int position) {
    ...
    holder.walletbalance.setText(ExchangeCalculator.getInstance()
        .displayBalanceNicely(
            ExchangeCalculator.getInstance().convertRate(Math.abs(box.getAmount()), ExchangeCalculator.getInstance().getCurrent().getRate()))
        + " "
    ...
    holder.plusminus.setTextColor(context.getResources().getColor(box.getAmount() > 0 ? R.color.ether_received : R.color.ether_spent));
    holder.walletbalance.setTextColor(context.getResources().getColor(box.getAmount() > 0 ? R.color.ether_received : R.color.ether_spent));
    holder.container.setAlpha(1f);
    if (box.getConfirmationStatus() == 0) {
      holder.month.setText(R.string.unconfirmed);
      ...
      holder.month.setTextColor(context.getResources().getColor(R.color.normal_black));
    } else {
