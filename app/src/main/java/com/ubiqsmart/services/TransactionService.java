...
public class TransactionService extends IntentService {
      ...
      EtherscanAPI.getInstance().getNonceForAddress(fromAddress, new Callback() {
        ...
        @Override public void onResponse(Call call, final Response response) throws IOException {
          try {
            ...
            RawTransaction tx = RawTransaction.createTransaction(nonce, new BigInteger(gas_price), new BigInteger(gas_limit), toAddress,
                new BigDecimal(amount).multiply(ExchangeCalculator.ONE_ETHER).toBigInteger(), data);
      ...
      @Override public void onResponse(Call call, final Response response) throws IOException {
        ...
        try {
          ...
          try {
            ...
            if (errormsg.indexOf(".") > 0) errormsg = errormsg.substring(0, errormsg.indexOf("."));
  ...
  private void suc(String hash) {
    builder.setContentTitle(getString(R.string.notification_transfersuc))
  ...
  private void error(String err) {
    builder.setContentTitle(getString(R.string.notification_transferfail))
        .setProgress(100, 100, false)
        .setOngoing(false)
        .setAutoCancel(true)
        .setContentText(err);
