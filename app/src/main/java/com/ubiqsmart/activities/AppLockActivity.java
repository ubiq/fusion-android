...
public class AppLockActivity extends BasePatternActivity implements PatternView.OnPatternListener, FingerprintListener {
  ...
  @RequiresApi(api = Build.VERSION_CODES.M) public void setupFingerprintStuff() {
  ...
  @RequiresApi(api = Build.VERSION_CODES.M) protected void generateKey() {
    try {
  ...
  @Override public void onPatternDetected(List<PatternView.Cell> pattern) {
    if (sharedPreferences.getLong("WRONG_PATTERN_LOCK", 0) != 0
        && sharedPreferences.getLong("WRONG_PATTERN_LOCK", 0) > System.currentTimeMillis() - 60 * 1000) {
  ...
  @Override protected void onPause() {
    ...
    if (fingerprintHelper != null && hasFingerprintSupport()) fingerprintHelper.cancel();
  }
  ...
  @Override public void onResume() {
    ...
    if (fingerprintHelper != null && hasFingerprintSupport()) setupFingerprintStuff();
  }
  ...
  protected void onWrongPattern() {
    ...
    if (numFailedAttempts >= 5) {
      ...
      editor.commit();
      mmessagetext.setText(R.string.locked_one_minute);
      numFailedAttempts = 0;
    }
  }
  ...
  @Override public void authenticationFailed(String error) {
  ...
  @RequiresApi(api = Build.VERSION_CODES.M) @Override public void authenticationSucceeded(FingerprintManager.AuthenticationResult result) {
