        ...
        @Override public void onResponse(Call call, final Response response) throws IOException {
          ...
          try {
            ...
            for (int i = 0; i < data.length(); i++) {
              if (!preferences.getString(data.getJSONObject(i).getString("account"), data.getJSONObject(i).getString("balance"))
                  .equals(data.getJSONObject(i).getString("balance"))) {
                if (new BigInteger(
                    preferences.getString(data.getJSONObject(i).getString("account"), data.getJSONObject(i).getString("balance"))).compareTo(
                    new BigInteger(data.getJSONObject(i).getString("balance"))) < 1) { // Nur wenn höhere Balance als vorher
                  notify = true;
                  address = data.getJSONObject(i).getString("account");
                  amount = amount.add(
                      (new BigInteger(data.getJSONObject(i).getString("balance")).subtract(new BigInteger(preferences.getString(address, "0")))));
                }
              }
