  ...
  companion object {
    ...
    init {
      try {
        ...
        if (position != -1) {
          Log.i("LinuxSecureRandom", "Secure randomness will be read from {} only.")
        } else {
          Log.i("LinuxSecureRandom", "Randomness is already secure.")
        }
      } catch (e: FileNotFoundException) {
        // Should never happen.
        Log.e("LinuxSecureRandom", "/dev/urandom does not appear to exist or is not openable")
        throw RuntimeException(e)
      } catch (e: IOException) {
        Log.e("LinuxSecureRandom", "/dev/urandom does not appear to be readable")
        throw RuntimeException(e)
      }
